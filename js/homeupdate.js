...
function gamesAdded() {
  fetch("/assets/json/links_all.json")
    ...
    .then((data) => countGames(data));

    function countGames(data) {
      let tileCount = 0;
      let gamesNumber = document.querySelector(".gamesNumber");
      for (const gamesList of data.links) {for (const game of gamesList.games) {tileCount++;}}
      gamesNumber.innerHTML = tileCount;
    }
}
